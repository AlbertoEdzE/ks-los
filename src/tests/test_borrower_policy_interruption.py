...
def test_borrower_policy_question_does_not_break_income_capture(monkeypatch):
    client = TestClient(app)
    session_id = "test-session-policy-income"
    ...
    resp = client.post(f"/api/v3/conversations/{session_id}/messages", json={"content": "350000, 25%"})
    assert resp.status_code == 200

    resp = client.post(
        f"/api/v3/conversations/{session_id}/messages",
        json={"content": "salaried, what is the minimum suggested income?"},
    )
    assert resp.status_code == 200
    data = resp.json()
    assert "minimum income requirement" in (data.get("response") or "").lower()

    resp = client.post(f"/api/v3/conversations/{session_id}/messages", json={"content": "10000"})
    assert resp.status_code == 200
    data = resp.json()
    text = (data.get("response") or "").lower()

    assert "monthly income" not in text
    assert ("how much are you looking to borrow" in text) or ("looking to borrow" in text) or ("loan amount" in text)
