...
from src.agents.nodes_legacy import (
    journey_coach_node,
    # tool_node,  # DISABLED - old workflow being phased out
    risk_engine_node,
    advisory_node,
    profile_parser_node
)
...
# Fallback for disabled tool_node
tool_node = None
...
def build_graph():
    ...
    # Only add tools node if available (old workflow being phased out)
    ...
    # Only add conditional edges if tools available
    if tool_node is not None:
        ...
        workflow.add_edge("journey_coach", "profile_parser")
